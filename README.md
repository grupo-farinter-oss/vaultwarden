...
- Push to `patched` publishes `ghcr.io/grupo-farinter-oss/vaultwarden:testing`.
- Push a tag that matches the release filter, for example `1.37.1` or `1.37.1-patched.2`, publishes `latest` and that release tag.
...
git checkout patched
git pull
git tag 1.37.1-patched.2
git push origin 1.37.1-patched.2
```
